# Copyright 2026 Kushan Shah
# SPDX-License-Identifier: Apache-2.0

"""Core Video Compositor for the GeoStream Renderer Microservice.

Handles WMS tile fetching, alpha-channel compositing, orbital tracking,
HUD rendering, and H.264 video encoding. This module is imported by the
FastAPI endpoint and executed in a thread pool to avoid blocking the event loop.

Caching Strategy:
    1. Check TieredCache (Redis L1 → Disk L2)
    2. On miss, fetch from NASA GIBS WMS API
    3. Write response to both cache tiers
"""

import cv2
import numpy as np
import requests
import datetime
import concurrent.futures
import sys
import json
import os
import hashlib
import subprocess
import logging

from cache import TieredCache

logger = logging.getLogger("renderer.compositor")

WIDTH = 1280
HEIGHT = 720


# ──────────────────────────────────────────────
# Tile Fetching with Tiered Cache
# ──────────────────────────────────────────────

def fetch_wms_image(url_template, layer_name, bbox, date_str, is_base, cache: TieredCache):
    """Fetch a WMS image tile with tiered caching (Redis L1 → Disk L2 → Network L3).
    Base layers = JPEG (opaque), overlays = PNG (with alpha).
    Non-GIBS servers skip the TIME param."""
    base_url = url_template.split("?")[0] if "?" in url_template else url_template
    fmt = "image/jpeg" if is_base else "image/png"
    is_gibs = "gibs.earthdata.nasa.gov" in url_template
    time_param = f"&TIME={date_str}" if (is_gibs and "BlueMarble" not in layer_name) else ""
    url = (
        f"{base_url}?SERVICE=WMS&REQUEST=GetMap&VERSION=1.1.1"
        f"&LAYERS={layer_name}&SRS=EPSG:4326"
        f"&BBOX={bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]}"
        f"&WIDTH={WIDTH}&HEIGHT={HEIGHT}&FORMAT={fmt}{time_param}"
        f"&TRANSPARENT={'FALSE' if is_base else 'TRUE'}"
    )

    # Cryptographic cache key
    url_hash = hashlib.md5(url.encode("utf-8")).hexdigest()
    ext = ".jpg" if is_base else ".png"
    cache_key = f"{url_hash}{ext}"

    # L1 + L2: Check tiered cache
    cached = cache.get(cache_key)
    if cached and len(cached) > 500:
        arr = np.frombuffer(cached, np.uint8)
        img = cv2.imdecode(arr, cv2.IMREAD_UNCHANGED)
        if img is not None:
            if img.ndim == 2:
                img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
            return img

    # L3: Network fetch on cache miss
    max_retries = 3
    for attempt in range(max_retries):
        try:
            timeout = 20 + (attempt * 10)
            resp = requests.get(url, headers={"User-Agent": "GeoStream-Renderer/1.0"}, timeout=timeout)

            if resp.status_code != 200:
                logger.warning(f"WMS {resp.status_code} for {layer_name} on {date_str} (attempt {attempt + 1})")
                continue

            content_type = resp.headers.get("Content-Type", "")
            if "xml" in content_type or "html" in content_type:
                logger.warning(f"WMS returned {content_type} for {layer_name} on {date_str}")
                return None

            if len(resp.content) < 500:
                logger.warning(f"WMS response too small ({len(resp.content)}B) for {layer_name}")
                continue

            # Write to tiered cache (both Redis and disk)
            cache.set(cache_key, resp.content)

            arr = np.frombuffer(resp.content, np.uint8)
            img = cv2.imdecode(arr, cv2.IMREAD_UNCHANGED)
            if img is not None:
                if img.ndim == 2:
                    img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
                return img
            logger.warning(f"cv2.imdecode failed for {layer_name} on {date_str}")

        except requests.exceptions.Timeout:
            logger.warning(f"Timeout ({timeout}s) for {layer_name} on {date_str} (attempt {attempt + 1})")
        except Exception as e:
            logger.warning(f"Fetch error for {layer_name}: {e} (attempt {attempt + 1})")

    logger.error(f"All {max_retries} attempts failed for {layer_name} on {date_str}")
    return None


# ──────────────────────────────────────────────
# Alpha Compositing
# ──────────────────────────────────────────────

def alpha_composite(bg, fg, is_overlay=True, layer_opacity=1.0):
    """Composites foreground over background using exact alpha channel or threshold."""
    if fg is None:
        return bg
    if bg is None:
        return fg[:, :, :3].copy() if fg.ndim == 3 else cv2.cvtColor(fg, cv2.COLOR_GRAY2BGR)

    if fg.shape[:2] != bg.shape[:2]:
        fg = cv2.resize(fg, (bg.shape[1], bg.shape[0]))

    bg_f = bg.astype(np.float32)

    if not is_overlay:
        return fg[:, :, :3].copy() if fg.ndim == 3 else cv2.cvtColor(fg, cv2.COLOR_GRAY2BGR)

    if fg.ndim == 3 and fg.shape[2] == 4:
        fg_rgb = fg[:, :, :3].astype(np.float32)
        alpha = (fg[:, :, 3] / 255.0).astype(np.float32)[:, :, np.newaxis] * layer_opacity
        out = alpha * fg_rgb + (1.0 - alpha) * bg_f
        return np.clip(out, 0, 255).astype(np.uint8)

    fg_rgb = fg[:, :, :3].astype(np.float32)
    brightness = fg_rgb.sum(axis=2)
    alpha = np.where(brightness > 10, 1.0, 0.0).astype(np.float32)[:, :, np.newaxis] * layer_opacity
    out = alpha * fg_rgb + (1.0 - alpha) * bg_f
    return np.clip(out, 0, 255).astype(np.uint8)


def calculate_activity(img):
    """Calculates percentage of active (non-transparent/non-black) pixels."""
    if img is None:
        return 0.0
    if img.ndim == 2:
        active = np.count_nonzero(img > 3)
    elif img.shape[2] == 4:
        active = np.count_nonzero(img[:, :, 3] > 0)
    else:
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        active = np.count_nonzero(gray > 3)
    return (active / (WIDTH * HEIGHT)) * 100.0


# ──────────────────────────────────────────────
# HUD Overlay
# ──────────────────────────────────────────────

def draw_hud(frame, date_str, bbox, layers, history, max_days):
    """Draws aviation-style heads-up display overlay on the video frame."""
    overlay = frame.copy()
    cv2.rectangle(overlay, (0, 0), (WIDTH, 40), (0, 0, 0), -1)
    hud_h = max(120, len(layers) * 20 + 20)
    cv2.rectangle(overlay, (0, HEIGHT - hud_h), (WIDTH, HEIGHT), (0, 0, 0), -1)
    cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)

    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(frame, "BATCH N-LAYER WMS RENDER (MP4)", (20, 25), font, 0.6, (50, 150, 255), 2, cv2.LINE_AA)
    cv2.putText(frame, f"DATE: {date_str}", (WIDTH - 200, 25), font, 0.6, (255, 255, 255), 1, cv2.LINE_AA)

    lat = (bbox[1] + bbox[3]) / 2
    lon = (bbox[0] + bbox[2]) / 2
    cv2.putText(frame, f"TGT COORD: {lat:.4f}, {lon:.4f}", (WIDTH // 2 - 120, 25), font, 0.5, (200, 200, 200), 1, cv2.LINE_AA)

    for i, layer in enumerate(layers):
        y_pos = HEIGHT - hud_h + 20 + (i * 20)
        name = layer["name"] if isinstance(layer, dict) else layer.name
        cv2.putText(frame, f"L{i + 1}: {name}", (20, y_pos), font, 0.5, (200, 255, 200), 1, cv2.LINE_AA)

    if history:
        graph_w, graph_h = 400, 80
        gx = WIDTH - graph_w - 20
        gy = HEIGHT - graph_h - 20
        cv2.rectangle(frame, (gx, gy), (gx + graph_w, gy + graph_h), (50, 50, 50), 1)
        cv2.putText(frame, "Top Layer Activity %", (gx, gy - 8), font, 0.4, (200, 200, 200), 1, cv2.LINE_AA)
        max_val = max(1.0, max(history) * 1.2)
        points = []
        for i, val in enumerate(history):
            x = gx + int((i / max(1, max_days - 1)) * graph_w)
            y = gy + graph_h - int((val / max_val) * graph_h)
            points.append((x, y))
        for i in range(1, len(points)):
            cv2.line(frame, points[i - 1], points[i], (0, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, f"{history[-1]:.2f}%", (gx + graph_w - 60, gy + 20), font, 0.5, (0, 255, 255), 1, cv2.LINE_AA)


# ──────────────────────────────────────────────
# Main Render Pipeline
# ──────────────────────────────────────────────

def render_video(request, cache: TieredCache) -> dict:
    """Core rendering pipeline. Accepts a RenderRequest and returns video path + metrics.
    
    This function is CPU-bound and should be called from a thread pool executor
    to avoid blocking the FastAPI async event loop."""

    bbox = request.bbox
    start_date = datetime.datetime.strptime(request.start_date, "%Y-%m-%d")
    end_date = datetime.datetime.strptime(request.end_date, "%Y-%m-%d")
    end_date = end_date.replace(hour=23, minute=59, second=59)

    freq = request.time_step.lower()
    freq_map = {
        "1d": datetime.timedelta(days=1),
        "1h": datetime.timedelta(hours=1),
        "10m": datetime.timedelta(minutes=10),
        "1m": datetime.timedelta(minutes=1),
    }
    freq_delta = freq_map.get(freq, datetime.timedelta(days=1))

    time_range = end_date - start_date
    data_steps = max(1, int(time_range / freq_delta) + 1)
    total_steps = min(request.frame_count, data_steps)
    delta = freq_delta if total_steps == data_steps else time_range / max(1, total_steps)

    layers = [{"url": l.url, "name": l.name, "opacity": l.opacity} for l in request.layers]
    if not layers:
        raise ValueError("No WMS layers provided")

    out_dir = os.path.dirname(request.output_path)
    if out_dir and not os.path.exists(out_dir):
        os.makedirs(out_dir)

    # Initialize video writer
    out = None
    for codec in ["mp4v", "avc1", "XVID"]:
        fourcc = cv2.VideoWriter_fourcc(*codec)
        out = cv2.VideoWriter(request.output_path, fourcc, request.fps, (WIDTH, HEIGHT))
        if out.isOpened():
            break
        out.release()

    if out is None or not out.isOpened():
        raise RuntimeError("Failed to initialize video writer with any codec")

    frames_per_step = max(1, request.frame_count // max(1, total_steps))

    # Orbital tracking setup
    tracking_mode = False
    tle_line1, tle_line2 = None, None
    sat_name = request.track_satellite.upper().replace(" ", "_") if request.track_satellite else ""
    if sat_name:
        try:
            sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "..", "scripts"))
            from orbital_tracker import fetch_tle, propagate, compute_tracking_bbox, SATELLITE_CATALOG
            norad_id = SATELLITE_CATALOG.get(sat_name)
            if norad_id:
                _, tle_line1, tle_line2 = fetch_tle(norad_id)
                if tle_line1 and tle_line2:
                    tracking_mode = True
                    logger.info(f"Orbital tracking enabled: {sat_name}")
        except ImportError as e:
            logger.warning(f"orbital_tracker module not available: {e}")

    activity_history = []
    last_good_frame = None
    last_good_date = None

    current_date = start_date
    step = 0

    while current_date <= end_date and step < total_steps:
        if delta.total_seconds() >= 86400:
            date_str = current_date.strftime("%Y-%m-%d")
        else:
            date_str = current_date.strftime("%Y-%m-%dT%H:%M:%SZ")

        frame = None
        sat_telemetry = None
        active_bbox = bbox

        if tracking_mode and tle_line1 and tle_line2:
            from orbital_tracker import propagate, compute_tracking_bbox
            sat_telemetry = propagate(tle_line1, tle_line2, current_date)
            if sat_telemetry:
                active_bbox = compute_tracking_bbox(sat_telemetry["lat"], sat_telemetry["lon"])

        # Concurrent tile fetching
        futures_map = {}
        with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, len(layers))) as executor:
            for idx, layer in enumerate(layers):
                is_base = idx == 0
                future = executor.submit(
                    fetch_wms_image, layer["url"], layer["name"], active_bbox, date_str, is_base, cache
                )
                futures_map[future] = idx

        layer_images = [None] * len(layers)
        for future in concurrent.futures.as_completed(futures_map):
            idx = futures_map[future]
            layer_images[idx] = future.result()

        for idx, img in enumerate(layer_images):
            if img is not None:
                opacity = layers[idx].get("opacity", 1.0)
                frame = alpha_composite(frame, img, is_overlay=(idx > 0), layer_opacity=opacity)

        top_img = layer_images[-1] if layer_images else None
        metric = calculate_activity(top_img)
        activity_history.append(metric)

        is_gap = False
        if frame is not None:
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) if frame.ndim == 3 else frame
            if np.mean(gray) > 3:
                last_good_frame = frame.copy()
                last_good_date = date_str
            else:
                if last_good_frame is not None:
                    frame = last_good_frame.copy()
                    is_gap = True
        elif last_good_frame is not None:
            frame = last_good_frame.copy()
            is_gap = True
        else:
            frame = np.zeros((HEIGHT, WIDTH, 3), dtype=np.uint8)
            cv2.putText(frame, f"AWAITING DATA: {date_str}", (WIDTH // 2 - 200, HEIGHT // 2),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 100, 255), 2)

        if is_gap:
            overlay = frame.copy()
            cv2.rectangle(overlay, (WIDTH - 380, 48), (WIDTH, 80), (0, 0, 180), -1)
            cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)
            gap_text = f"DATA GAP - Showing {last_good_date}" if last_good_date else "DATA GAP"
            cv2.putText(frame, gap_text, (WIDTH - 375, 70),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 200, 200), 1, cv2.LINE_AA)

        frame = frame.astype(np.uint8)
        draw_hud(frame, date_str, active_bbox, layers, activity_history, total_steps)

        # Satellite telemetry HUD
        if tracking_mode and sat_telemetry:
            font = cv2.FONT_HERSHEY_SIMPLEX
            overlay = frame.copy()
            cv2.rectangle(overlay, (WIDTH - 320, 45), (WIDTH, 170), (0, 0, 0), -1)
            cv2.addWeighted(overlay, 0.75, frame, 0.25, 0, frame)
            cv2.putText(frame, f"TRACKING: {sat_name}", (WIDTH - 310, 65), font, 0.5, (0, 255, 100), 1, cv2.LINE_AA)
            cv2.putText(frame, f"ALT: {sat_telemetry['alt_km']:.1f} km", (WIDTH - 310, 85), font, 0.4, (0, 200, 255), 1, cv2.LINE_AA)
            cv2.putText(frame, f"VEL: {sat_telemetry['velocity_km_s']:.3f} km/s", (WIDTH - 310, 103), font, 0.4, (0, 200, 255), 1, cv2.LINE_AA)
            cv2.putText(frame, f"LAT: {sat_telemetry['lat']:.4f}", (WIDTH - 310, 121), font, 0.4, (200, 200, 200), 1, cv2.LINE_AA)
            cv2.putText(frame, f"LON: {sat_telemetry['lon']:.4f}", (WIDTH - 310, 139), font, 0.4, (200, 200, 200), 1, cv2.LINE_AA)
            cv2.putText(frame, f"INC: {sat_telemetry['inclination_deg']:.2f} deg", (WIDTH - 310, 157), font, 0.4, (180, 180, 180), 1, cv2.LINE_AA)

        for _ in range(frames_per_step):
            out.write(frame)

        current_date += delta
        step += 1

        # Progress reporting via stdout (consumed by Go pipeline or FastAPI)
        current_frames = min(request.frame_count, step * frames_per_step)
        print(json.dumps({"progress": current_frames, "total": request.frame_count}), flush=True)

    out.release()

    # FFmpeg re-encode to H.264 for browser compatibility
    temp_path = request.output_path.replace(".mp4", "_raw.mp4")
    os.rename(request.output_path, temp_path)
    try:
        ffmpeg_cmd = [
            "ffmpeg", "-y", "-i", temp_path,
            "-c:v", "libx264", "-preset", "fast", "-crf", "23",
            "-pix_fmt", "yuv420p",
            "-movflags", "+faststart",
            request.output_path,
        ]
        result = subprocess.run(ffmpeg_cmd, capture_output=True, text=True, timeout=120)
        if result.returncode != 0:
            logger.warning(f"FFmpeg re-encode failed: {result.stderr}")
            os.rename(temp_path, request.output_path)
        else:
            os.remove(temp_path)
            logger.info("FFmpeg re-encode to H.264 successful")
    except FileNotFoundError:
        logger.warning("FFmpeg not found, using raw OpenCV output")
        os.rename(temp_path, request.output_path)
    except subprocess.TimeoutExpired:
        logger.warning("FFmpeg timed out, using raw OpenCV output")
        if os.path.exists(temp_path):
            os.rename(temp_path, request.output_path)

    return {
        "video_path": request.output_path,
        "metrics": activity_history,
    }
